...
function(copySDL2DLL BUILD_TYPE)
    message(STATUS "Copying SDL2 DLL")
    # Copy
    # "${OGRE_BINARIES}/Dependencied/bin/${BUILD_TYPE}/SDL2.dll" to "${CMAKE_SOURCE_DIR}/bin/${BUILD_TYPE}
    # and the other DLLs as well. On non-Windows machines, we can only the DLLs for the current build.
    if (WIN32 OR OGRE_BUILD_TYPE_MATCHES)
        # Lists of DLLs to copy
        set(SDL2_DLL
        ...
        if (EXISTS "${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${SDL2_DLL}${DLL_OS_SUFFIX}")
            file(COPY "${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${SDL2_DLL}${DLL_OS_SUFFIX}"
                 DESTINATION "${CMAKE_SOURCE_DIR}/bin/${BUILD_TYPE}")
        else ()
            message(STATUS "SDL2 DLL not found")
            message(STATUS "Trying SDL2 path: ${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${SDL2_DLL}${DLL_OS_SUFFIX}")
        endif ()
    endif ()
endfunction()
...
function(copyAMDAGSDLL BUILD_TYPE PLATFORM_X64)
    message(STATUS "Copying amd ags DLL")
    # Copy
    # "${OGRE_BINARIES}/Dependencied/bin/${BUILD_TYPE}/SDL2.dll" to "${CMAKE_SOURCE_DIR}/bin/${BUILD_TYPE}
    # and the other DLLs as well. On non-Windows machines, we can only the DLLs for the current build.
    if (WIN32 OR OGRE_BUILD_TYPE_MATCHES)
        # Lists of DLLs to copy
        set(Dependencies_DLL
                amd_ags_uwp
                amd_ags
            )

        # Deal with platform x64
        if (${PLATFORM_X64})
            set(PLATFORM_SUFFIX "_x64")
        else ()
            set(PLATFORM_SUFFIX "_x32")
        endif ()
        ...
        foreach(DLL_NAME ${Dependencies_DLL})
            message(STATUS "\tTarget DLL: ${DLL_NAME}")
            if (EXISTS "${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${DLL_NAME}${PLATFORM_SUFFIX}${DLL_OS_SUFFIX}")
                file(COPY "${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${DLL_NAME}${PLATFORM_SUFFIX}${DLL_OS_SUFFIX}"
                     DESTINATION "${CMAKE_SOURCE_DIR}/bin/${BUILD_TYPE}")
            else ()
                message(STATUS "\t${DLL_NAME} DLL not found")
                message(STATUS "\tTrying ${DLL_NAME} path: ${OGRE_DLL_PATH}/${DLL_OS_PREFIX}${DLL_NAME}${PLATFORM_SUFFIX}${DLL_OS_SUFFIX}")
            endif ()
        endforeach()
    endif ()
endfunction()
...
macro(setupOgre OGRE_SOURCE, OGRE_BINARIES, OGRE_LIBRARIES_OUT,
    ...
    if (WIN32)
        ...
        link_directories("${OGRE_BINARIES}/lib/${CMAKE_BUILD_TYPE}")
    elseif (APPLE)
        set(OGRE_BINARIES "${OGRE_SOURCE}/build" CACHE STRING "Path to OGRE's build folder generated by CMake")
    ...
    copyAMDAGSDLL(${CMAKE_BUILD_TYPE} ${PLATFORM_X64})
...
macro(addStaticDependencies OGRE_SOURCE, OGRE_BINARIES, OGRE_BUILD_SETTINGS_STR, OGRE_DEPENDENCY_LIBS)
    ...
    if (WIN32)
        set(OGRE_DEPENDENCIES "${OGRE_SOURCE}/Dependencies/lib/${CMAKE_BUILD_TYPE}" CACHE STRING
